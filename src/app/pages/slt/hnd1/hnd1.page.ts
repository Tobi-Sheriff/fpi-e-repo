...
export class Hnd1Page {
  ...
  fileUploadFS(event: FileList) {
    ...
    const fileStoragePath = `filesStorage/SLT/HND1/FS/${new Date().getTime()}_${file.name}`;
  ...
  fileUploadSS(event: FileList) {
    ...
    const fileStoragePath = `filesStorage/SLT/HND1/SS/${new Date().getTime()}_${file.name}`;
