...
export class Nd2Page {
  ...
  constructor(
    ...
    this.ngFirestoreCollectionSS = angularFirestore.collection<FILE>('filesCollectionSLTND2SS');
  ...
  fileUploadFS(event: FileList) {
    ...
    const fileStoragePath = `filesStorage/SLT/ND2/FS/${new Date().getTime()}_${file.name}`;
  ...
  fileUploadSS(event: FileList) {
    ...
    const fileStoragePath = `filesStorage/SLT/ND2/SS/${new Date().getTime()}_${file.name}`;
